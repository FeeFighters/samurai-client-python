...
Note that if a payment\_method and True for update are passed into get\_transparent\_redirect\_form\_initial, Samurai will use the values submitted in that previous payment\_method for every required field that is submitted as blank in the new form. In addition, the same applies to credit card number and CVV if they are invalid, which means it's safe to populate the fields with "*" characters for the unknown digits; the "*"s make it invalid, so the previous values will be used. For convenience, in the template `samurai\_form.card\_number.value` (samurai\_form being the example template variable holding the form values, yours may be different) and `samurai\_form.verification\_value` will hold the appropriate values with the unknown digits replaced by "*"s.
